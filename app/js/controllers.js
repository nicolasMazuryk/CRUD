...
angular.module( 'crudAppControllers', [])
    ...
    .controller( 'likesTplCtrl', [ '$rootScope', '$scope', '$http', function( $rootScope, $scope, $http ) {
        var vm = this;
        ...
        $http.get( 'http://localhost:4000/likes')
            .success( function ( data ) {
                ...
                $rootScope.storageLength = data.storageLength;
    ...
    .controller( 'newsRequestCtrl', [ '$rootScope', '$scope', '$http', 'Categories', function( $rootScope, $scope, $http, Categories ) {
        var music = this;

        $scope.addToLiked = function ( event ) {
            //$http.post( 'http://localhost:4000/post', event )
            $http.post( 'http://localhost:4000/post', JSON.stringify( event ) )
                .success( function ( res ) {
                    console.log( res );
                    $rootScope.storageLength += 1;
                })
                .error( function ( error ) {
                    console.log( error );
                });
